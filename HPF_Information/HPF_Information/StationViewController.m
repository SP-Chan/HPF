...
@interface StationViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[DataBaseUtil shareDataBaseUtil]createTableWithTableName:@"Station"];
...
-(HPFBaseImageView *)iconImageView
{
    if (!_iconImageView) {
        ...
        _iconImageView.image = [UIImage imageNamed:@"icon_zhandian"];
    }
...
-(UITextField *)textFild
{
    if (!_textFild) {
        ...
        _textFild.placeholder = @"  请输入公交站名称:";
...
-(void)delectHistory
{
    [[DataBaseUtil shareDataBaseUtil]deleteMessageWithTableName:@"Station"];
    [_tab reloadData];
}
-(UITableView *)tab
{
...
{
    return [[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Station"].count;
}
...
{
    StationResultViewController *stationResult = [[StationResultViewController alloc] init];
    NSArray *array = [[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Station"];
    stationResult.busName = [array objectAtIndex:array.count-1-indexPath.row];
    [self.navigationController pushViewController:stationResult animated:YES];
}
...
{
    if ([[[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Station"] containsObject:_textFild.text]) {
        
    }else
    {
        [[DataBaseUtil shareDataBaseUtil] addMessage:_textFild.text tableName:@"Station"];;
        [_tab reloadData];
    }
