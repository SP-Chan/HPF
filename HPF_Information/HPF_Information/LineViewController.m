...
@interface LineViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[DataBaseUtil shareDataBaseUtil]createTableWithTableName:@"Line"];
...
-(void)delectHistory
{
    [[DataBaseUtil shareDataBaseUtil]deleteMessageWithTableName:@"Line"];
    [_tab reloadData];
}
-(UITableView *)tab
{
...
{
    return [[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Line"].count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *array = [[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Line"];
    cell.textLabel.text = [array objectAtIndex:array.count-1 - indexPath.row];
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    LineResultViewController *lineResult = [[LineResultViewController alloc] init];
    NSArray *array = [[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Line"];
    lineResult.busNumber = [array objectAtIndex:array.count-1-indexPath.row];
    [self.navigationController pushViewController:lineResult animated:YES];
}
...
-(HPFBaseImageView *)iconImageView
{
    if (!_iconImageView) {
        ...
        _iconImageView.image = [UIImage imageNamed:@"icon_luxian.png"];
    }
...
{
    if ([[[DataBaseUtil shareDataBaseUtil] seleteBusesWithTableName:@"Line"] containsObject:_textFild.text]) {
        
    }else
    {
        [[DataBaseUtil shareDataBaseUtil] addMessage:_textFild.text tableName:@"Line"];;
        [_tab reloadData];
    }

    if (_textFild.text.length > 0) {
        LineResultViewController *result = [[LineResultViewController alloc] init];
        result.busNumber = _textFild.text;

        [self.navigationController pushViewController:result animated:YES];
    }else{
        _alert = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"请正确线路信息" preferredStyle:UIAlertControllerStyleAlert];
