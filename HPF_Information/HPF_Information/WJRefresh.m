...
- (void)endRefresh{
    ...
    NSLog(@"====>%f",self.RefreshTableView.contentSize.height);
   
    self.timeLb.text = [NSString stringWithFormat:@"最后更新时间 %@",[self getNowTime]];
    [self.refreshLoadingView stopAnimating];
    [UIView animateWithDuration:0.25 animations:^{
        if (self.isFootFreshing) {
                self.frame = CGRectMake(0, 0,self.frame.size.width, WJRefreshDropHeight);
            }
...
- (UIActivityIndicatorView *)refreshLoadingView{
    if (_refreshLoadingView == nil) {
        ...
        _refreshLoadingView.frame = CGRectMake((WJRefreshScreenW-100)/2-55, point.y - size.height/2, size.width, size.height);
        _refreshLoadingView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhite;
        [_refreshLoadingView setColor:[UIColor blackColor]];
...
- (UIImageView *)arrowImageView{
    if (_arrowImageView == nil) {
        _arrowImageView = [[UIImageView alloc]initWithFrame:CGRectMake((WJRefreshScreenW-100)/2-45, 0, 15, WJRefreshDropHeight)];
...
- (UILabel *)tipLb{
    if (_tipLb == nil) {
        _tipLb = [[UILabel alloc]initWithFrame:CGRectMake((WJRefreshScreenW-200)/2, 5, 200, 15)];
...
- (UILabel *)timeLb{
    if (_timeLb == nil) {
        _timeLb = [[UILabel alloc]initWithFrame:CGRectMake((WJRefreshScreenW-200)/2, 20, 200, 15)];
