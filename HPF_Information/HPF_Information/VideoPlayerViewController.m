...
-(void)creatPlayer
{
    NSString *urls =@"http://qiqu.uc.cn/api/video?id=";
    
    NSString *url = [urls stringByAppendingString:self.MP4Url];
    NSLog(@"%@",url);
    
    NSURL *uu =[NSURL URLWithString:url];
    
    self.playerItem = [[AVPlayerItem alloc]initWithURL:uu];;
...
-(void)creatBackViewAndTopView
{
    ...
    self.topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT * 0.15)];
...
- (void)createProgress
{
    self.progress = [[UIProgressView alloc]initWithFrame:CGRectMake(55, SCREEN_HEIGHT - 22 , SCREEN_WIDTH * 0.7, 15)];
...
- (void)createSlider
{
     self.slider = [[UISlider alloc]initWithFrame:CGRectMake(52, SCREEN_HEIGHT - 29 , SCREEN_WIDTH * 0.68 , 15)];
...
{
   self.currenTimeLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH *0.84, SCREEN_HEIGHT - 30, 100, 20)];
...
{    //返回按钮
    ...
    [Backbutton setBackgroundImage:[UIImage imageNamed:@"fanh.png"] forState:UIControlStateNormal];
...
- (void)createTitle
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH* 0.1,0, SCREEN_WIDTH * 0.76, SCREEN_HEIGHT * 0.15)];
